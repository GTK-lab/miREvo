...
## options
my %options=();
getopts("ug:v:f:ck:os:t:er:q:dx:zy:ab:p:V:i:",\%options);
...
if(not $options{'t'}){
    print STDERR "\nmake_html_simple.pl for mirdeep2: No organism specified by switch -u [organism] so UCSC browser BLAT will not be used.
...
if($options{'c'}){
	$csv = 1;
    open CSV,">$prefix/predict.result.csv" or die "Error: cannot create csv file\n";
}
...
## open output files
open HTML,">$prefix/predict.result.html" or die "Error: cannot create html file\n";
...
#### read in all micrornas in data
        ...
        open HTML,">$prefix/predict.expression.html" or die "cannot create expression.html\n";
        CreateHTML(); ##
